...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    char user[BUF_SIZE + 1];
    int num_read = read(client_fd, user, BUF_SIZE);
    if (num_read == -1) {
        perror("server: read");
        exit(1);
    }
    usernames[user_index].username = malloc(sizeof(char)*num_read);
    strcpy(usernames[user_index].username, user);
    return client_fd;
}
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    char *usr = usernames[client_index].username;
    char buf[BUF_SIZE + 1];
    char write_buf[BUF_SIZE + 1];

    strncpy(write_buf, usr, strlen(usr));
    strcat(write_buf, ": ");
    ...
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (usernames[i].sock_fd != -1) {
            if (num_read == 0 || write(usernames[i].sock_fd, write_buf, strlen(write_buf)) != strlen(write_buf)) {
                usernames[client_index].sock_fd = -1;
                return fd;
            }
        }
    }
    // if (num_read == 0 || write(fd, write_buf, strlen(write_buf)) != strlen(write_buf)) {
    //     usernames[client_index].sock_fd = -1;
    //     return fd;
    // }
    return 0;
