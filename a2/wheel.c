...
char **prune_word_list(char **words, int len, int *words_remaining) {
    char **new_words;
    int i, j, size, total_size;
    i = j = size = total_size = 0;
    while (words[i]) {
        // All strings null terminated, safe
    ...
    new_words = malloc((sizeof(char *) * (size + 1)));
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    while (valid) {
        ...
        int conv_input = strtol(input, NULL, 10);
        if (conv_input == 0) {
            printf("There are no words of that length.\n");
        } else {
            *len = conv_input;
            ...
            pruned_words = prune_word_list(words, *len, &words_remaining);
            if (!words_remaining) {
                printf("There are no words of that length.\n");
                deallocate_pruned_word_list(pruned_words);
                valid = 1;
            }
        }
    }
